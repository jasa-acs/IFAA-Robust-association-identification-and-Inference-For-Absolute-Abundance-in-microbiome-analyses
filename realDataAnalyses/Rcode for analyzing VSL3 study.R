#
## R code for analyzing the VLS#3 study data
#

realDataSets=get(load("realData.RData"))
data_VSL3=realDataSets$data_VSL3

#-------------------------------------------------------------------
## IFAA method to analyze the VSL3 study data
#-------------------------------------------------------------------
...
runIFAA=function(
  ...
  IFAAresul=IFAA(MicrobData=w,CovData=xData,linkIDname="id",
                 testCov=c("x1","x2"),nRef=40,nPermu=40,
                 reguMethod="mcp",
                 refReadsThresh=0.2,SDThresh=0,
                 SDquantilThresh=0,balanceCut=0,
                 fwerRate=fwer,bootB=500,seed=1)
...
VSL3resu=runIFAA(data=data_VSL3,Mprefix="rawCount",
                 covsPrefix="x",fwer=0.25)

VSL3resu$analysisResults


#-------------------------------------------------------------------
## ANCOM method to analyze the VSL3 study data.
...
ancomResul=runAncom(data=data_VSL3,Mprefix="rawCount",groupVar="x1")
ancomResul$TaxaNamByAncom

#-------------------------------------------------------------------
## Spearman test method to analyze the VSL3 study data.
#-------------------------------------------------------------------
...
SpearmanTest(data=data_VSL3,AAprefix="rawCount",xVar="x1",adjAlpha=0.25)
